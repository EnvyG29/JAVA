...
public class MainClassDz3 {
    ...
    private static List<ArrayList<String>> ex2(List<ArrayList<String>> data_base, String genre, String name) {
        /**
         ...
         */
        ArrayList<String> li = new ArrayList<>(Arrays.asList(genre));
        for (int i = 0; i < data_base.size(); i++) {
            if (genre == data_base.get(i).get(0)) {
                data_base.get(i).add(name);
                return data_base;
            }
        }
        data_base.add(li);
